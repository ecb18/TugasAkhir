...
def getBookById(barcodeid):
    response = requests.post("http://127.0.0.1:8000/api/RaspberryPi/find/book", data=barcodeid, headers=headers)
...
def sendData(data):
    response = requests.post("http://127.0.0.1:8000/api/RaspberryPi/borrow/store", data=data, headers=headers)
    print(response.text)

def verifyLastBorrow(id):
    response = requests.post("http://127.0.0.1:8000/api/RaspberryPi/borrow/history",data=id, headers=headers)
    data = json.loads(response.text)
    if (response.status_code == 404):
